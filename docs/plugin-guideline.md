...
* **Format:** Use Reverse DNS notation: `{DOMAIN}.{PRODUCT}` / `com.{author}.{plugin-name}` (e.g. `com.smok3y97.tilemetrics.beszel`).
* **Action UUID Prefix:** Every action UUID **must** be prefixed with your plugin's root UUID (e.g. `com.smok3y97.tilemetrics.beszel.cpu`).
...
* Accurately reflects plugin functionality (e.g. `"TileMetrics (Beszel)"`).
...
* **Action Names:** Concise (<= 30 characters), clear, and specific (e.g. `"CPU"`, `"Memory"`, `"Storage"`, `"Network"`, `"GPU"`, `"UPS"`).
...
* **Input Elements:**
  ...
  * Provide immediate validation feedback on invalid inputs.
* **Layout & Content:**
